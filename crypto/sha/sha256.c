...
static void			hash_loop(uint32_t *x, uint32_t *words)
{
	...
	while (i < 64)
	{
		y[0] = x[7] + epsilon2562(x[4]);
		y[0] += choose256(x[4], x[5], x[6]) + g_k[i] + words[i];
		y[1] = epsilon2561(x[0]) + majority256(x[0], x[1], x[2]);
		x[7] = x[6];
		x[6] = x[5];
		x[5] = x[4];
		x[4] = x[3] + y[0];
...
static void			prepare(uint32_t *words, uint32_t *msg)
{
	size_t i;
	...
	{
		if (i < 16)
			words[i] = bswap32(msg[i]);
		else
		{
			words[i] = sigma2561(words[i - 2]) + words[i - 7];
			words[i] += sigma256(words[i - 15]) + words[i - 16];
		}
		i++;
	}
}
